window.PORTFOLIO_CONTENT = {
  "hero": {
    "kicker": "AI Vibe Coding / Photography Portfolio",
    "title": "Focus the frame. Enter the work.",
    "description": "我用 AI 做实验，用文字整理判断，用摄影保存观看。这个 portfolio 从一次对焦开始。"
  },
  "heroPhotos": [
    {
      "src": "assets/photos/inbox/pacific-sunset.jpg",
      "alt": "海边日落与潮线"
    },
    {
      "src": "assets/photos/inbox/snow-peak-mono.jpg",
      "alt": "黑白雪山与强光"
    },
    {
      "src": "assets/photos/inbox/westminster-dusk.jpg",
      "alt": "黄昏下的伦敦天际线"
    },
    {
      "src": "assets/photos/inbox/rain-window.jpg",
      "alt": "雨天车窗后的城市"
    },
    {
      "src": "assets/photos/inbox/platform-light.jpg",
      "alt": "阳光下的空站台"
    },
    {
      "src": "assets/photos/inbox/quiet-cyclist.jpg",
      "alt": "街口骑车的人"
    },
    {
      "src": "assets/photos/inbox/red-geraniums-inbox.jpg",
      "alt": "暗处的红色花朵"
    }
  ],
  "workPanels": [
    {
      "number": "01",
      "title": "AI Vibe Coding",
      "description": "把模糊想法变成小工具、界面原型和可复盘的创作流程。",
      "href": "projects.html"
    },
    {
      "number": "02",
      "title": "Process & Interaction",
      "description": "把摄影里的取景、对焦、曝光、显影，转译成网页进入和滚动体验。",
      "href": "projects.html"
    },
    {
      "number": "03",
      "title": "Photography",
      "description": "用系列作品、街头观察和商业拍摄整理我的观看方式。",
      "href": "photography.html",
      "image": "assets/photos/tree-canopy.jpg",
      "alt": "树冠之间的天空"
    }
  ],
  "projects": [
    {
      "title": "Recruiting Any",
      "meta": "AI Work / Local-first Sourcing Platform / 2026",
      "href": "https://github.com/alexlxy599/recruiting-any",
      "description": "一个从 Discover 到 Pool 再到 Outreach 的 AI sourcing 实验，把研究、搜索、候选人资料和个性化触达放进同一个本地工作流。"
    },
    {
      "title": "Personal OS Dashboard",
      "meta": "Interface / Personal System / 2026",
      "href": "#",
      "description": "把日程、灵感、任务和写作片段聚合成一个低噪音的个人界面。"
    },
    {
      "title": "AI Photography Critic",
      "meta": "Research / Photography Critique / 2025",
      "href": "#",
      "description": "尝试让模型从构图、色彩、叙事和观看路径上给照片做结构化反馈。"
    }
  ],
  "photoBreak": {
    "src": "assets/photos/inbox/pacific-sunset.jpg",
    "alt": "海边日落与潮线",
    "text": "A practice of attention, patience, and finishing things with care."
  },
  "photos": [
    {
      "title": "Platform",
      "location": "Montreal",
      "layout": "wide",
      "src": "assets/photos/inbox/subway-platform.jpg",
      "alt": "地铁站台上的人影"
    },
    {
      "title": "Crowd Study",
      "location": "Montreal",
      "layout": "tall",
      "src": "assets/photos/inbox/subway-crowd.jpg",
      "alt": "地铁站里的俯视人群"
    },
    {
      "title": "Gallery Image",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/gallery-57a9d980.jpg",
      "alt": "横幅摄影作品"
    },
    {
      "title": "Gallery Image",
      "location": "Vancouver",
      "layout": "tall",
      "src": "assets/photos/inbox/gallery-9346ae20.jpg",
      "alt": "竖幅摄影作品"
    },
    {
      "title": "Gulls",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/gulls-at-sunset.jpg",
      "alt": "日落时飞过的人群与海鸟"
    },
    {
      "title": "Through Glass",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/sunset-frame.jpg",
      "alt": "窗框里的海上日落"
    },
    {
      "title": "Roof Gull",
      "location": "Vancouver",
      "layout": "tall",
      "src": "assets/photos/inbox/roof-gull.jpg",
      "alt": "屋檐上的海鸥"
    },
    {
      "title": "Vertical Sun",
      "location": "Vancouver",
      "layout": "tall",
      "src": "assets/photos/inbox/vertical-sun.jpg",
      "alt": "竖幅海上日落"
    },
    {
      "title": "Amber Sun",
      "location": "Vancouver",
      "layout": "tall",
      "src": "assets/photos/inbox/amber-sun.jpg",
      "alt": "金色海面上的落日"
    },
    {
      "title": "Coastal Evening",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/coastal-evening.jpg",
      "alt": "傍晚的海岸线"
    },
    {
      "title": "Violet Coast",
      "location": "Vancouver",
      "layout": "tall",
      "src": "assets/photos/inbox/violet-coast.jpg",
      "alt": "紫色暮光里的海岸"
    },
    {
      "title": "Red Light",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/red-light-figure.jpg",
      "alt": "红色光线照亮的人物"
    },
    {
      "title": "Winter Mountain",
      "location": "Fairbanks",
      "layout": "wide",
      "src": "assets/photos/inbox/winter-mountain.jpg",
      "alt": "雪地中的山脉"
    },
    {
      "title": "Winter Road",
      "location": "Fairbanks",
      "layout": "wide",
      "src": "assets/photos/inbox/winter-road.jpg",
      "alt": "冬日道路上的夕阳"
    },
    {
      "title": "Street Elder",
      "location": "London",
      "layout": "wide",
      "src": "assets/photos/inbox/street-elder.jpg",
      "alt": "阳光街道上的老人"
    },
    {
      "title": "Wheel Field",
      "location": "London",
      "layout": "tall",
      "src": "assets/photos/inbox/wheel-field.jpg",
      "alt": "草地远处的摩天轮"
    },
    {
      "title": "Village Facade",
      "location": "Cambridge",
      "layout": "tall",
      "src": "assets/photos/inbox/village-facade-new.jpg",
      "alt": "阳光下的白色乡村建筑"
    },
    {
      "title": "Market Shade",
      "location": "London",
      "layout": "wide",
      "src": "assets/photos/inbox/market-stroller.jpg",
      "alt": "市场棚下的人和婴儿车"
    },
    {
      "title": "Window Sign",
      "location": "London",
      "layout": "tall",
      "src": "assets/photos/inbox/window-sign.jpg",
      "alt": "窗边标识和室内光线"
    },
    {
      "title": "Tree Building",
      "location": "London",
      "layout": "wide",
      "src": "assets/photos/inbox/tree-building.jpg",
      "alt": "树枝与建筑立面"
    },
    {
      "title": "Canal Window",
      "location": "Cambridge",
      "layout": "tall",
      "src": "assets/photos/inbox/canal-window.jpg",
      "alt": "窗框中的水边景色"
    },
    {
      "title": "Church Tower",
      "location": "London",
      "layout": "tall",
      "src": "assets/photos/inbox/church-tower.jpg",
      "alt": "蓝天下的教堂塔楼"
    },
    {
      "title": "Evening Pair",
      "location": "London",
      "layout": "tall",
      "src": "assets/photos/inbox/evening-couple.jpg",
      "alt": "黄昏里靠在一起的人"
    },
    {
      "title": "Fireworks",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/fireworks-bridge.jpg",
      "alt": "桥边夜空中的烟花"
    },
    {
      "title": "Sailboat",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/sailboat-swimmer.jpg",
      "alt": "海面上的帆船与游泳者"
    },
    {
      "title": "Birds Between Buildings",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/birds-between-buildings.jpg",
      "alt": "楼宇间飞过的鸟"
    },
    {
      "title": "Lake Silhouette",
      "location": "Vancouver",
      "layout": "tall",
      "src": "assets/photos/inbox/lake-silhouette.jpg",
      "alt": "湖边的人影和山"
    },
    {
      "title": "Snow Negative",
      "location": "Fairbanks",
      "layout": "wide",
      "src": "assets/photos/inbox/snow-negative.jpg",
      "alt": "黑白反相质感的雪山"
    },
    {
      "title": "Quiet Building",
      "location": "Tokyo",
      "layout": "wide",
      "src": "assets/photos/inbox/quiet-building.jpg",
      "alt": "安静光线里的建筑"
    },
    {
      "title": "Wind Window",
      "location": "Tokyo",
      "layout": "wide",
      "src": "assets/photos/inbox/wind-window.jpg",
      "alt": "车窗外的风车与暮色"
    },
    {
      "title": "Vending Machines",
      "location": "Tokyo",
      "layout": "wide",
      "src": "assets/photos/inbox/vending-machines.jpg",
      "alt": "街边自动售货机"
    },
    {
      "title": "Gallery Image",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/img-4216.jpg",
      "alt": "横幅摄影作品"
    },
    {
      "title": "Gallery Image",
      "location": "Vancouver",
      "layout": "wide",
      "src": "assets/photos/inbox/img-4821.jpg",
      "alt": "横幅摄影作品"
    },
    {
      "title": "Gallery Image",
      "location": "Vancouver",
      "layout": "tall",
      "src": "assets/photos/inbox/img-5321.jpg",
      "alt": "竖幅摄影作品"
    }
  ],
  "about": {
    "kicker": "About",
    "title": "一个在 AI、文字和摄影之间来回试东西的人。",
    "description": "我关心工具如何改变创作，也关心人如何在更快的技术节奏里保留自己的观看、判断和手感。这个网站会持续更新我的项目、文章和摄影系列。"
  },
  "links": [
    {
      "label": "Email",
      "href": "mailto:hello@example.com"
    },
    {
      "label": "GitHub",
      "href": "#"
    },
    {
      "label": "Instagram",
      "href": "#"
    }
  ],
  "photoCities": [
    {
      "city": "London",
      "key": "london",
      "map": "assets/maps/map-london.jpg",
      "description": "Streets, stone, transit, and late light."
    },
    {
      "city": "Vancouver",
      "key": "vancouver",
      "map": "assets/maps/map-vancouver.jpg",
      "description": "Water, coastlines, city edges, and evening color."
    },
    {
      "city": "Fairbanks",
      "key": "fairbanks",
      "map": "assets/maps/map-fairbanks.jpg",
      "description": "Snow, roads, mountains, and northern light."
    },
    {
      "city": "Tokyo",
      "key": "tokyo",
      "map": "assets/maps/map-cambridge.jpg",
      "description": "Small urban observations and moving windows."
    },
    {
      "city": "Kyoto",
      "key": "kyoto",
      "map": "assets/maps/map-kyoto.jpg",
      "description": "Temples, streets, gardens, and quiet structure."
    },
    {
      "city": "Cambridge",
      "key": "cambridge",
      "map": "assets/maps/map-tokyo.jpg",
      "description": "Rivers, colleges, village facades, and soft light."
    }
  ]
};
